...
#define FIFO          "par-shell-in"
#define EXIT_GLOBAL   "exit-global"
#define EXIT          "exit"
#define STATS         "stats"
#define NEW_TERMINAL  "new_terminal" 
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if(strcmp(args[0], EXIT_GLOBAL) == 0) {
      lista_pid *list;
      for(list = lista->first; list!=NULL; list = list->next){
    ...
    if (strcmp(args[0], EXIT) == 0) {
      destroi_processo(lista, atoi(args[1]));
      cont--;
      if(cont == 0){
    ...
    if (strcmp(args[0], STATS) == 0) {
      ...
      sprintf_file(buffer, "temp_fifo_%d", atoi(args[1]));

      entrada = open_file(buffer, ESCREVER);

      if(sprintf(buffer, "%d %d\n", num_children, total) < 0){
    ...
    if (strcmp(args[0], NEW_TERMINAL) == 0) {
      insere_novo_processo(lista, atoi(args[1]));
      cont++;
      continue;
    }
